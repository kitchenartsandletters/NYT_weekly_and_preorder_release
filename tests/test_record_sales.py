...
from refactor.src.record_sales import record_sales
...
def fetch_sales(conn):
    return [dict(r) for r in conn.execute("SELECT isbn, order_id, quantity FROM sales_log ORDER BY id").fetchall()]
...
def test_regular_only(conn):
    order = sample_order()
    inserted = record_sales(order)
    assert inserted is True
    rows = fetch_sales(conn)
    assert rows == [
...
def test_mixed_preorder_and_regular(conn):
    conn.execute("INSERT INTO preorders (isbn) VALUES ('111')")
    order = sample_order()
    inserted = record_sales(order)
    assert inserted is True
    rows = fetch_sales(conn)
    # only second item should be inserted
    assert rows == [
        {"isbn": "222", "order_id": "o1", "quantity": 2},
    ]


def test_preorder_only(conn):
    conn.execute("INSERT INTO preorders (isbn) VALUES ('111')")
    conn.execute("INSERT INTO preorders (isbn) VALUES ('222')")
    order = sample_order()
    inserted = record_sales(order)
...
def test_duplicate_order(conn):
    order = sample_order()
    record_sales(order)
    record_sales(order)
    rows = fetch_sales(conn)
    assert len(rows) == 2
    # dedup ensures no duplicates: row count remains 2 (not 4)
    record_sales(order)
    rows_after = fetch_sales(conn)
